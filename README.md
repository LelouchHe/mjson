...
本来试图让删除操作返回被删除的元素,但是这样有内存泄露的风险,因为返回给调用方的对象,我没有办法控制,如果调用方不主动fini的话,那部分内容铁定泄露了

所以就决定先阻止调用方获得不在json控制之外的对象

## 目前状态

现在mjson基本开发完毕了,有几点还是没有完善,但鉴于我本周开始要专心致志的找工作了(其实很久就开始了,结果中间插入了mjson啊摔..)
* writer还是很低效,没有实现上面提的is_dirty优化
* get_str以来的底层搞定了,但mjson的接口还没有实现(分分钟的事)
* 重要接口之一迭代器还没有实现,不过这个应该快,毕竟map/vec的迭代都搞定了.可能唯一的难点是迭代器的结构,和能否封装成通过的格式
* 性能和内存评测.虽然目前很远大,但还没有实地测试下具体的指标

当然,要想开发好用舒服的json库,还有很多事情要做,set/get相等性判断,各种细节的小优化,都还没有开始
